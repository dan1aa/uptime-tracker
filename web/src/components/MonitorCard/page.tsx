...
import { actions, MonitorItem } from '@/store';
    ...
    <Card className={`${className} h-fit`}>
        <CardHeader>
            ...
            <div className='flex w-full h-fit gap-x-2 items-center'>
                <div className={`w-3 h-3 rounded-full ${STATUS_CONFIG[monitor.status].dotClass}`}></div>
                <span>{STATUS_CONFIG[monitor.status].message}</span>
            </div>
        </CardHeader>
        <CardContent>
            <div className='flex items-center w-fit h-fit gap-x-2'>
                <Button variant={"default"}>View</Button>
                <Button variant={"destructive"} onClick={() => { actions.removeMonitor(monitor.id) }}>Remove</Button>
            </div>
        </CardContent>
    </Card>
  )
