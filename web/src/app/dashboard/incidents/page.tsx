...
function Incidents() {
  ...
  const filteredIncidents: IncidentItem[] = snap.incidents.filter(
    (incident: IncidentItem) => {
      const currentMonitor: MonitorItem | undefined = snap.monitors.find(
        (monitor: MonitorItem) => monitor.id == incident.monitorId,
      );
      const matchesSearch = currentMonitor?.title
        .toLowerCase()
        .includes(snap.ui.searchIncidentsQuery.toLowerCase());

      const matchesResolved =
        snap.ui.incidentsFilter === "all" ||
        incident.resolved === snap.ui.incidentsFilter;
  ...
  return (
    <div className="flex-1 min-h-0 overflow-y-auto pr-1">
      ...
      {filteredIncidents.length ? (<div className="w-full flex h-fit items-center justify-between pl-4 py-2 border-b border-solid border-gray-500/15">
        <div className="w-1/4 h-fit">
          <Badge variant="outline" className="py-1 px-3 h-7 text-sm">
            Title
          </Badge>
        </div>
        <div className="w-1/4 h-fit">
      ...
      {filteredIncidents.length ? (
          ...
          return (
            ...
            >
              <div className="w-1/4 h-fit flex items-center gap-x-3">
                <div
                  className={`${incident.resolved == "resolved" ? "bg-status-live" : "bg-status-down"} w-4 h-4 rounded-full`}
                ></div>
                <span>
                  {
                    snap.monitors.find(
        ...
        <EmptySearch
          ...
          message="No incidents found, all of your sites are live!"
          Icon={FolderIcon}
        />
      )}
    </div>
